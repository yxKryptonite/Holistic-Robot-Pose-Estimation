...
def draw_projected_box(img, points, color=(0, 255, 0), thickness=2):
    ...
    # based on the generation order in DreamWriter
    # Bottom face (z_min)
...
def draw_keypoints(img, keypoints, color=(0, 255, 255)):
    ...
    for kp in keypoints:
        ...
        if loc:
            ...
            cv2.putText(img, kp['name'], center, cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
...
def main():
    ...
    for json_path in data_files:
        ...
        for obj in data.get("objects", []):
            ...
            if "panda" not in obj_class:
                continue

            # Color: Robot = Green, Distractors = Red
            ...
            # Draw keypoints
            if "keypoints" in obj and "panda" in obj_class.lower():
